...
|---|---|---|---|
| Remote | Fast SSH/server coding | Neovim, tmux, OpenCode, Git/GitHub CLI, shell PATH, Node 24, focused search/build tools | GUI terminals, desktop automation, Herdr/Pi, containers, Kubernetes, cloud/infra, work/AWS configs |
| Minimal | Focused workstation | Remote + Pi, Herdr, rich shell UX, Starship, zoxide, bat/eza, lazygit, navi, television, sesh, ast-grep, terminal config/install when not headless | Kubernetes/container/cloud/infra stack, AWS/work-specific config, desktop automation extras |
...
Profiles are forward-management choices. Switching from `full` to `minimal` or `remote` stops managing excluded targets and stops installing their profile tools on future runs; it does not uninstall existing tools or delete existing files.
...
Set `DOTFILES_INCLUDE_SECRETS=1` and run the bootstrap with `--with-secrets` when you want secret targets verified/applied safely.
...
On Debian/Ubuntu, the first apply installs system packages with `sudo`. `gh` is installed in every profile, but GitHub authentication is optional and separate from 1Password. Public installs are attempted unauthenticated; if GitHub rate limits a mise plugin, run `gh auth login` or export `GITHUB_TOKEN`, then retry `"$HOME/.local/bin/mise" install`.
...
5. Run `"$HOME/.local/bin/mise" install` or `chezmoi apply` to install.
